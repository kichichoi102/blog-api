...
export default (app: Router) => {
  ...
  app.get('/comments', (req: Request, res: Response) => {
    ...
    if (req.query.postId) {
      commentRes = comments.filter(p => p.postId == parseInt(req.query.postId as string));
    }
    if (req.query.email) {
      commentRes = commentRes.filter(p => p.email == (req.query.email as string));
    }
  ...
  app.get('/comments/:id', (req: Request, res: Response) => {
    ...
    if (!comment) {
      res.send(`The Comment with id: ${req.params.id} was not found`).status(404);
    }
